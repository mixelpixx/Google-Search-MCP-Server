...
// API Validation Function - checks SerpAPI connectivity at startup
async function validateSerpAPI(): Promise<{ valid: boolean; warning?: string }> {
  ...
  if (!apiKey) {
    ...
    console.error('💡 Free tier: 100 searches/month');
    console.error('💡 Paid tier: $50/month for 5,000 searches\n');
    return { valid: false };
  }

  try {
    // Test API with a simple query
    const testUrl = `https://serpapi.com/search?api_key=${apiKey}&engine=google&q=test&num=1`;
    ...
    if (response.ok) {
      ...
      console.error('✅ SerpAPI credentials validated successfully');
      return { valid: true };
    }

    if (response.status === 401 || response.status === 403) {
      console.error('❌ Invalid API Key (401/403)');
      console.error('   Check your SERPAPI_KEY in .env file');
      console.error('   Get your API key: https://serpapi.com/manage-api-key');
      return { valid: false };
    }

    if (response.status === 429) {
      console.error('⚠️  API Rate Limit Hit (you may have used all 100 free searches this month)');
      console.error('   Server will still start, but searches will fail until next month');
      console.error('   Or upgrade your plan: https://serpapi.com/pricing');
      return { valid: true, warning: 'rate_limit' };
    }

    if (response.status === 400) {
      console.error('❌ Invalid API Request (400)');
      console.error('   Check your SERPAPI_KEY');
      return { valid: false };
    }
    ...
    if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
      console.error('❌ Network error - cannot reach SerpAPI');
...
class GoogleSearchServerV3 {
  ...
  async start(validation?: { valid: boolean; warning?: string }) {
    try {
      const transportType = process.env.MCP_TRANSPORT || 'stdio';

      console.error('');
      console.error('='.repeat(60));
      console.error('Google Search MCP Server v4.2.0 (SerpAPI)');
      console.error('='.repeat(60));
      ...
      if (validation?.warning === 'rate_limit') {
        console.error('');
        console.error('⚠️  WARNING: Rate limit detected during validation');
        console.error('   You may have used your 100 free searches this month');
        console.error('   Searches will fail until quota resets next month');
        console.error('   Or upgrade your plan at https://serpapi.com/pricing');
        console.error('');
      }
...
// Main entry point - validates API before starting server
async function main() {
  console.error('Validating SerpAPI credentials...\n');
  const validation = await validateSerpAPI();
  ...
  // API is valid, now start the server
  const server = new GoogleSearchServerV3();
  await server.start(validation);
}
