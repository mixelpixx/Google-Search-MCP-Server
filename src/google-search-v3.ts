  ...
  async start(validation?: { valid: boolean; warning?: string }) {
    try {
      ...
      if (validation?.warning === 'rate_limit') {
        ...
        console.error('   You may have used your 100 free queries today');
        console.error('   Searches will fail until quota resets at midnight UTC');
        console.error('');
      }
...
async function main() {
  console.error('Validating Google API credentials...\n');
  const validation = await validateGoogleAPI();

  if (!validation.valid) {
    console.error('\n❌ Server startup aborted due to API validation failure');
    console.error('   Fix the issues above and try again');
    console.error('\n💡 IMPORTANT: Google closed this API to new customers in 2024');
    console.error('   If you don\'t have existing credentials, see ALTERNATIVES.md\n');
    process.exit(1);
  }
